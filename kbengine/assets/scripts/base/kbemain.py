...
import os
serverinfo=		TServerInfoList()
def readxml():	
	path2=os.path.abspath('.')  
	xml = XML2Dict()
	serverlist_picker=TServer_INFO_LIST_PICKLER()
	server_picker=	SERVER_INFO_PICKLER();
	serverlist=	[];	
	result = xml.parse(path2+'/res/server/serverinfo.xml')
	for data in result.root.server:				
			serverlist.append([data.serverid,data.name])			
	serverinfo=	serverlist_picker.createObjFromDict({"values":serverlist})
